...
export function initBoardHandlers(boardService: BoardService, userId: string): void {
  ...
  ipcMain.handle('node:update', async (_, id: string, updates: Partial<Node>) => {
    try {
      return await boardService.updateNode(id, updates)
    } catch (error) {
  ...
  ipcMain.handle('node:delete', async (_, id: string) => {
    try {
      return await boardService.deleteNode(id)
    } catch (error) {
  ...
  ipcMain.handle('relation:update', async (_, id: string, updates: Partial<NodeRelation>) => {
    try {
      return await boardService.updateRelation(id, updates)
    } catch (error) {
  ...
  ipcMain.handle('relation:delete', async (_, sourceId: string, targetId: string) => {
    try {
      return await boardService.deleteRelation(sourceId, targetId)
    } catch (error) {
